...
本工具用于抓取和查询百度贴吧的帖子与用户数据，数据缓存于 `~/.cache/tieba/`，支持跨贴吧查询。
...
```bash
# 拉取「天堂鸡汤」最近50个帖子
tieba fetch 天堂鸡汤 -n 50

# 按回复时间排序，强制刷新
tieba fetch 天堂鸡汤 --sort reply --refresh
```
...
# 查询「天堂鸡汤」最近3天，显示发言正文
tieba query --fname 天堂鸡汤 --days 3 --detail
...
```bash
# 1. 先拉取数据（可定期执行）
tieba fetch 天堂鸡汤 -n 30

# 2. 查询近期活跃用户概览
tieba query --fname 天堂鸡汤 --brief

# 3. 定向查找可疑用户
tieba query --ip 海外 --days 30

# 4. 导出 JSON 供进一步分析
tieba query --fname 天堂鸡汤 --days 0 --json > users.json
```
