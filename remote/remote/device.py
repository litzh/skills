...
def cmd_add(args) -> None:
    ...
    manual_ip = getattr(args, "ip", None)
    manual_mac = getattr(args, "mac", None)

    if manual_ip or manual_mac:
        # Manual mode: both --ip and --mac are required
        if not manual_ip or not manual_mac:
            print("Error: --ip and --mac must both be provided for manual add.")
            return
        mac = manual_mac.upper().replace("-", ":")
        ip = manual_ip
        ip_mode = getattr(args, "ip_mode", "static")
        model = getattr(args, "model", None) or ""
        print(f"Manual add: {mac}  {ip}  {model or '(no model)'}")
        ans = input(f"Save as '{name}' (ip_mode={ip_mode})? [Y/n] ").strip().lower()
        if ans not in ("", "y"):
            print("Cancelled.")
            return
    else:
